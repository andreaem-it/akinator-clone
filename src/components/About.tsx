...
export function About() {
  return (
    <section id="about" className="relative mx-auto max-w-6xl px-6 py-28 sm:px-10 sm:py-36">
      <Reveal>
        <span className="text-sm text-(--color-lime)">Profilo</span>
      </Reveal>
      ...
      <div className="mt-20 grid grid-cols-1 gap-x-8 gap-y-12 sm:grid-cols-2">
        {skillGroups.map((group, i) => (
          <Reveal key={group.title} delay={0.05 * i}>
            <h3 className="text-sm uppercase tracking-widest text-(--color-ink-dim)">
              {group.title}
            </h3>
            <ul className="mt-4 flex flex-wrap gap-2">
              {group.items.map((item) => (
                <li
                  key={item}
                  className="rounded-full border border-(--color-void-line) px-4 py-2 text-sm transition-colors hover:border-(--color-lime) hover:text-(--color-lime)"
                >
                  {item}
                </li>
              ))}
